...
ISO_DIR_IMG="${ISO_DIR}/iso"
ISO_DIR_RO="${ISO_DIR_TMP}/ro"
ISO_DIR_RW="${ISO_DIR_TMP}/rw"
ISO_DIR_MNT="${ISO_DIR_TMP}/mnt"
ISO_SRC="${ISO_DIR_IMG}/${ISO_NAME}.iso"
ISO_DST="${ISO_DIR_IMG}/${ISO_NAME}-ks.iso"
KS_FILE="${ISO_DIR}/kickstart_el8.cfg"

if ! rpm -q xorriso > /dev/null 2>&1; then
	dnf install -y xorriso
fi

for dir in "${ISO_DIR}" "${ISO_DIR_TMP}" "${ISO_DIR_IMG}" "${ISO_DIR_RO}" "${ISO_DIR_RW}" "${ISO_DIR_MNT}"; do
	[ -d "${dir}" ] || mkdir -v "${dir}"
done
...
if ! mountpoint -q "${ISO_DIR_RO}"; then
	mount -t iso9660 -o loop "${ISO_SRC}" "${ISO_DIR_RO}"
else
	echo "${ISO_DIR_RO} already mounted, please check."
	exit 1
fi
...
for modfile in 'isolinux/isolinux.cfg' 'EFI/BOOT/grub.cfg' 'images/efiboot.img'; do
	...
	/bin/cp -av "${ISO_DIR_RO}/${modfile}" "${ISO_DIR_RW}/${modfile}"
	sync
	if [[ "${modfile}" =~ .cfg$ ]]; then
		echo "Adding ks.cfg to ${modfile}"
		sed -r -i "s#(.*)(hd:LABEL=\S+)(.*)#\1\2 inst.ks=\2:/ks.cfg console=tty0 console=ttyS0,115200n8 net.ifnames.prefix=net\3#g" "${ISO_DIR_RW}/${modfile}"
		sed -i 's# quiet##g' "${ISO_DIR_RW}/${modfile}"
	fi
done
...
if [ ! -f "${ISO_DST}" ]; then
	...
	echo "${ISO_DST} already exists, please check."
fi
